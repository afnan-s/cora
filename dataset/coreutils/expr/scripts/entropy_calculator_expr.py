...
if __name__ == "__main__":
	...
	result1 = subprocess.run(['opt', '--O0', '-load', 'llvm-passes/build/libPerState.'+lib_extension, '-legacy-log-state', '-S', sut_ll_file, '-o', instrumented_file, '-enable-new-pm=0'], stdout=subprocess.PIPE)
		...
		if in_file_name.endswith(".in"):
			try: 	
				...
				with open(os.path.join(in_dir, in_file_name), 'r') as input_file:
					intput_expr = input_file.read().strip()
					# lli --extra-archive=libver.a --extra-archive=libcoreutils.a --extra-archive=libc_nonshared.a --dlopen=libgmp.so factor.ll
					# print(['lli', '--extra-archive=src/bin/libver.a --extra-archive=src/bin/libcoreutils.a --extra-archive=src/bin/libc_nonshared.a --dlopen=src/bin/libgmp.so', instrumented_file, intput_expr, '>', out_file_name])
					p = subprocess.Popen(['lli', '--extra-archive=src/bin/libver.a --extra-archive=src/bin/libcoreutils.a --extra-archive=src/bin/libc_nonshared.a --dlopen=src/bin/libgmp.so', instrumented_file, intput_expr, '>', out_file_name], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
