		...
		if in_file_name.endswith(".in"):
			try: 	
				...
				with open(os.path.join(in_dir, in_file_name), 'r') as input_file:
					...
					subprocess.call('lli --extra-archive=src/bin/libver.a --extra-archive=src/bin/libcoreutils.a --extra-archive=src/bin/libc_nonshared.a --dlopen=src/bin/libgmp.so '+instrumented_file+' '+intput_number+' > '+out_file_name, shell=True, timeout=1*60*60)
