...
if __name__ == "__main__":
	...
	for i in range(giveup):
		result1 = subprocess.run(['opt', '-load', 'llvm-passes/build/libPerState.'+lib_extension, '-legacy-per-state', '-S', sut_ll_file, '-o', perturbed_file, '-enable-new-pm=0'], stderr=subprocess.PIPE, universal_newlines=True)
	...
	for variant_file_name in os.listdir(perturbed_path):
		...
		for in_file_name in os.listdir(in_dir):
			...
			if in_file_name.endswith(".in"):
				...
				perturbed_out_file_name = os.path.join(in_dir,in_file_name)+"_pertubed.out"
				...
				for j in range(runs):
					p = subprocess.Popen(['lli', os.path.join(perturbed_path, variant_file_name), '-S _pertubed.out', os.path.join(in_dir,in_file_name) ], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
					# print(p)
