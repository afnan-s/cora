...
if __name__ == "__main__":
	...
	for in_file_name in os.listdir(in_dir):
		if in_file_name.endswith(".in"):
			try: 	
				print("Now running gzip over file "+ in_file_name+"\n")
				...
				out_file_name = os.path.join(in_dir, in_file_name)+".out"
				# p = subprocess.Popen(['lli', '--jit-kind=mcjit', instrumented_file, '-S .out', os.path.join(in_dir, in_file_name)], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
				p = subprocess.Popen(['lli', instrumented_file, '-S .out', os.path.join(in_dir, in_file_name)], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
