	...
	for variant_file_name in os.listdir(perturbed_path):
		...
		for in_file_name in sorted(os.listdir(in_dir), key=natsort):
			...
			if in_file_name.endswith(".in"):
				...
				for j in range(runs):
					p = subprocess.Popen(['lli', os.path.join(perturbed_path, variant_file_name), os.path.join(in_dir,in_file_name), perturbed_out_file_name ], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
					# print(p)
